...
def prepare_words_set(definitions: Iterable[str]) -> Iterable[Iterable[str]]:
    return [functions.filter_and_lemmatize_words_in(defs) for defs in definitions]
...
def main():
    ...
    definizioni: list = [""] * num_columns
    # pre-processing step
    for i in range(0, num_columns):
        definizioni[i] = prepare_words_set(lettore[i])

    similarities_to_test = 0
    similarity_functions_name = ["overlap", "jaccard"]
    for simil_name in similarity_functions_name:

        print("\n\n unsing the similarity function \'", simil_name,"\', the results are the following:")
            ...
            for y in range(0, lel):
                if x is not y:  # non confrontiamo gli elementi con se' stessi ...
                    for i in range(0, num_columns):
                        ddd = definizioni[i]
                        similarita[i].append(
                            functions.compute_sim(ddd[x], ddd[y], usingOverlapSimilarity=similarities_to_test == 0))
