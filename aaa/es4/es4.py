...
def get_preprocessed_words(text):
	return functions.preprocessing(text)
...
def document_segmentation(list_of_sentences):
	...
	option = functions.SynsetToBagOptions(bag=set())
	...
	for words in words_each_sentences:
		for w in words:
			...
			if w in all_words:
				...
				words_counts_and_bags[w] = [1, (w, cache.get_extended_bag_for_words(option=option))] #bag_of_word_expanded
